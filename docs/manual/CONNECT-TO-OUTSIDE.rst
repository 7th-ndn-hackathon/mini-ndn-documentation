...
Then the following lines can be added to ``bin/minindn`` before
net.start():
...
Adding these before net.start() is important as node "root" would then
be assigned an IP automatically. Re-install Mini-NDN by issuing the
following command in the mini-ndn folder:
...
Run Mini-NDN with the simple topology and issue ifconfig on the local
